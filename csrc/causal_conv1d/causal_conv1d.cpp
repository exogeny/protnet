...
void set_conv_params_bwd(ConvParamsBwd &params,
                         ...
                         // device pointers
                         const at::Tensor x, const at::Tensor weight,
                         void *bias_ptr, const at::Tensor dout,
                         const at::Tensor dx, const at::Tensor dweight,
                         void *dbias_ptr, bool silu_activation) {
  // Pass in "dout" instead of "out", we're not gonna use "out" at all.
  set_conv_params_fwd(params, batch, dim, seqlen, width, x, weight, dout,
                      bias_ptr, silu_activation);
...
causal_conv1d_bwd(const at::Tensor &x, const at::Tensor &weight,
                  ...
                  const c10::optional<at::Tensor> &dx_,
  ...
  TORCH_CHECK(x.stride(2) == 1 || x.stride(1) == 1);
  const bool is_channel_last = x.stride(1) == 1 && x.stride(2) > 1;
  if (!is_channel_last && dout.stride(2) != 1) {
  ...
  if (is_channel_last) {
    ...
    TORCH_CHECK(x.stride(2) % 8 == 0 and x.stride(0) % 8 == 0,
                "causal_conv1d with channel last layout requires strides "
                "(x.stride(0) and x.stride(2)) to be multiples of 8");
    TORCH_CHECK(dout.stride(2) % 8 == 0 and dout.stride(0) % 8 == 0,
                "causal_conv1d with channel last layout requires strides "
                "(dout.stride(0) and dout.stride(2)) to be multiples of 8");
  }
  ...
  if (dx_.has_value()) {
    ...
    if (!is_channel_last) {
      TORCH_CHECK(dx.stride(2) == 1);
    }
    if (is_channel_last) {
      TORCH_CHECK(dx.stride(1) == 1);
    }
  } else {
  ...
  at::Tensor dweight =
      torch::zeros_like(weight, weight.options().dtype(at::kFloat));
  at::Tensor dbias;
  if (bias_.has_value()) {
    dbias = torch::zeros_like(bias_.value(),
                              bias_.value().options().dtype(at::kFloat));
  }

  ConvParamsBwd params;
  set_conv_params_bwd(
      params, batch_size, dim, seqlen, width, x, weight,
      bias_.has_value() ? bias_.value().data_ptr() : nullptr, dout, dx, dweight,
      bias_.has_value() ? dbias.data_ptr() : nullptr, silu_activation);
  ...
  if (dfinal_states_.has_value()) {
    TORCH_CHECK(is_channel_last,
                "dfinal_states is only supported for channel last layout");
  ...
  if (return_dinitial_states) {
    dinitial_states =
        torch::zeros({batch_size, width - 1, dim}, x.options()).transpose(1, 2);
  ...
  auto stream = at::cuda::getCurrentCUDAStream().stream();
  DISPATCH_ITYPE_FLOAT_AND_HALF_AND_BF16(
      x.scalar_type(), "causal_conv1d_bwd", [&] {
        DISPATCH_WTYPE_FLOAT_AND_HALF_AND_BF16(
            weight.scalar_type(), "causal_conv1d_bwd", [&] {
              if (!is_channel_last) {
                causal_conv1d_bwd_cuda<input_t, weight_t>(params, stream);
              } else {
                causal_conv1d_channellast_bwd_cuda<input_t, weight_t>(params,
                                                                      stream);
              }
            });
      });
  return std::make_tuple(dx, dweight.to(weight.dtype()),
                         bias_.has_value() ? dbias.to(bias_.value().dtype())
                                           : dbias,
                         dinitial_states);
}
