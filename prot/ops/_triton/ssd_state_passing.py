...
def _state_passing_fwd(states, dA_chunk_cumsum, initial_states=None, seq_idx=None, chunk_size=None,
    ...
    out = torch.zeros((batch, nchunks, nheads, dim), device=states.device, dtype=out_dtype)
    final_states = torch.zeros((batch, nheads, dim), device=states.device, dtype=torch.float32)
...
def _state_passing_bwd(
    ...
    if seq_idx is not None:
        ...
        seqlen = seq_idx.shape[-1]
        assert seq_idx.shape == (batch, seqlen)
    dstates = torch.zeros_like(dout, dtype=dstates_dtype if dstates_dtype is not None else dout.dtype)
    if states_dtype is not None and states_dtype != states.dtype:
        states_converted = torch.zeros_like(states, dtype=dstates_dtype if dstates_dtype is not None else dout.dtype)
        assert states_converted.stride() == states.stride()
    else:
        states_converted = None
    if has_initial_states:
        dinitstates = torch.zeros_like(dstates[:, 0])
    else:
        dinitstates = None
    ...
    ddA_chunk_cumsum = torch.zeros(
        batch, nheads, nchunks, n_blocks,
    ...
    BLOCK_SIZE_actual = _state_passing_bwd_kernel.best_config.kwargs.get("BLOCK_SIZE")
