...
def _layer_norm_fwd(x, weight, bias, eps, z=None, out=None, group_size=None, norm_before_gate=True, is_rms_norm=False):
    ...
    mean = torch.zeros((ngroups * M, ), dtype=torch.float32, device=x.device) if not is_rms_norm else None
    rstd = torch.zeros((ngroups * M, ), dtype=torch.float32, device=x.device)
...
def _layer_norm_bwd(dy, x, weight, bias, eps, mean, rstd, z=None, group_size=None,
    ...
    _dw = torch.zeros((nrow_groups, N), dtype=torch.float32, device=weight.device)
    _db = torch.zeros((nrow_groups, N), dtype=torch.float32, device=bias.device) if bias is not None else None
