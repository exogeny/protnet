...
from prot.ops._c import causal_conv1d_cuda
from prot.ops.causal_conv1d_interface import causal_conv1d_fn

from prot.ops._triton.ssd_bmm import _bmm_chunk_fwd, _bmm_chunk_bwd
from prot.ops._triton.ssd_chunk_state import _chunk_cumsum_fwd, _chunk_cumsum_bwd
from prot.ops._triton.ssd_chunk_state import _chunk_state_fwd, _chunk_state_bwd_db
from prot.ops._triton.ssd_chunk_state import chunk_state, chunk_state_ref
from prot.ops._triton.ssd_chunk_state import chunk_state_varlen
from prot.ops._triton.ssd_state_passing import _state_passing_fwd, _state_passing_bwd
from prot.ops._triton.ssd_state_passing import state_passing, state_passing_ref
from prot.ops._triton.ssd_chunk_scan import _chunk_scan_fwd, _chunk_scan_bwd_dz, _chunk_scan_bwd_dstates
from prot.ops._triton.ssd_chunk_scan import _chunk_scan_bwd_dC, _chunk_scan_bwd_dcb
from prot.ops._triton.ssd_chunk_scan import _chunk_scan_bwd_ddAcs_stable
from prot.ops._triton.ssd_chunk_scan import chunk_scan, chunk_scan_ref
from prot.ops._triton.layer_norm_gated import rmsnorm_fn, _layer_norm_fwd, _layer_norm_bwd
from prot.ops._triton.k_activations import _swiglu_fwd, _swiglu_bwd
...
def selective_scan_bwd(dout, x, dt, A, B, C, D=None, z=None):
    ...
    """
    import prot.ops._triton.selective_scan as selective_scan
...
def ssd_selective_scan(x, dt, A, B, C, D=None, z=None, dt_bias=None, dt_softplus=False, dt_limit=(0.0, float("inf"))):
    ...
    Return:
        out: (batch, seqlen, nheads, headdim)
    """
    from prot.ops._triton.selective_scan import selective_scan_fn
...
class MambaSplitConv1dScanCombinedFn(torch.autograd.Function):
    ...
    def forward(
        ...
        xBC_conv = causal_conv1d_cuda.causal_conv1d_fwd(
        ...
        if rmsnorm_weight is None:
            ...
            if d_nonssm == 0:
                ...
                out01 = torch.zeros((batch, seqlen, d_nonssm + dim), dtype=x_rms.dtype, device=x_rms.device)
        ...
        ctx.save_for_backward(
            zxbcdt, conv1d_weight, conv1d_bias,
    ...
    def backward(ctx, dout, *args):
        ...
        if recompute_output:
            out_recompute = torch.zeros(*out.shape[:2], d_nonssm + dim, device=out.device, dtype=out.dtype)
        ...
        xBC_conv = rearrange(
            causal_conv1d_cuda.causal_conv1d_fwd(
                rearrange(xBC.contiguous(), "b s d -> b d s"),
        ...
        dxBC_given = rearrange(dxBC_given.contiguous(), "b s d -> b d s")
        dxBC_given, dweight, dbias, *_ = causal_conv1d_cuda.causal_conv1d_bwd(
            rearrange(xBC.contiguous(), "b s d -> b d s"),
            conv1d_weight,
            conv1d_bias,
            rearrange(dxBC.contiguous(), "b s d -> b d s"),
            seq_idx, None, None, dxBC_given, False,
