...
from prot.ops._triton.softplus import softplus
...
def _chunk_cumsum_bwd_kernel(
    ...
    if DT_SOFTPLUS:
        ddt = tl.where(dt_presoftplus <= 20.0, ddt * safe_sigmoid(dt_presoftplus), ddt)
...
def _chunk_cumsum_fwd(dt, A, chunk_size, dt_bias=None, dt_softplus=False, dt_limit=(0.0, float("inf"))):
    ...
    dA_cumsum = torch.zeros(batch, nheads, nchunks, chunk_size, device=dt.device, dtype=torch.float32)
...
def _chunk_state_fwd(B, x, dt, dA_cumsum, seq_idx=None, states=None, states_in_fp32=True):
    ...
    if states is not None:
        ...
        states = torch.zeros((batch, nchunks, nheads, headdim, dstate), device=x.device, dtype=states_dtype)
...
def _chunk_state_bwd_db(x, dt, dA_cumsum, dstates, seq_idx=None, B=None, ngroups=1):
    ...
    dB = torch.zeros(batch, seqlen, nsplits, ngroups, dstate, device=x.device, dtype=torch.float32)
...
def chunk_state_varlen(B, x, dt, dA_cumsum, cu_seqlens, chunk_states):
    ...
    states = torch.zeros(batch, nheads, headdim, dstate, dtype=chunk_states.dtype, device=chunk_states.device)
    grid = lambda META: (triton.cdiv(headdim, META['BLOCK_SIZE_M']) * triton.cdiv(dstate, META['BLOCK_SIZE_N']),
                    batch, nheads)
    with torch.cuda.device(x.device.index):
