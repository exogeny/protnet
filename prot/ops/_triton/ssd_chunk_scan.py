...
from prot.ops._triton import safe_sigmoid
...
def _chunk_scan_fwd(cb, x, dt, dA_cumsum, C, states, D=None, z=None, seq_idx=None):
    ...
    # Allocates output.
    out = torch.zeros(batch, seqlen, nheads, headdim, device=x.device, dtype=x.dtype)
    if z is not None:
        out_x = torch.zeros(batch, seqlen, nheads, headdim, device=x.device, dtype=x.dtype)
...
def _chunk_scan_bwd_dz(x, z, out, dout, chunk_size, has_ddAcs=True, D=None, dz=None, recompute_output=False):
    ...
    if recompute_output:
        outz = torch.zeros_like(x)
    dout_x = torch.zeros_like(dout)
...
def _chunk_scan_bwd_dstates(C, dA_cumsum, dout, seq_idx=None, dtype=None):
    ...
    dprev_states = torch.zeros(batch, nchunks, nheads, headdim, dstate, device=C.device, dtype=dtype)
...
def _chunk_scan_bwd_dC(prev_states, dA_cumsum, dout, seq_idx=None, C=None, ngroups=1):
    ...
    dC = torch.zeros(batch, seqlen, nsplits, ngroups, dstate, device=dout.device, dtype=torch.float32)
...
def _chunk_scan_bwd_dcb(x, dt, dA_cumsum, dout, seq_idx=None, CB=None, ngroups=1):
    ...
    nheads_per_program = max(min(math.ceil(batch * nchunks * nheads / sm_count), nheads_ngroups_ratio), 1)
    nsplits = triton.cdiv(nheads_ngroups_ratio, nheads_per_program)
    dcb = torch.zeros(batch, nchunks, nsplits, ngroups, chunk_size, chunk_size, device=x.device, dtype=torch.float32)
...
def _chunk_scan_bwd_dx(cb, x, dt, dA_cumsum, dout, D=None):
    ...
    # if D is not None:
    #     BLOCK_SIZE_M_min = 32
    #     dD = torch.zeros(triton.cdiv(chunk_size, BLOCK_SIZE_M_min), batch, nchunks, nheads, headdim, device=D.device, dtype=torch.float32)
    # else:
    #     dD = None
...
def _chunk_scan_bwd_ddAcs_unstable(x, dt, out, dout, ddt, D=None, subtract_ddtdt=True):
    ...
    if D is not None:
        assert D.shape == (nheads, headdim) or D.shape == (nheads,)
    ddA_cumsum = torch.zeros_like(dt)
