...
def _layer_norm_fwd(
    ...
    # allocate output
    y = torch.zeros_like(x, dtype=x.dtype if out_dtype is None else out_dtype)
    assert y.stride(-1) == 1
    if weight1 is not None:
    ...
    if return_dropout_mask and dropout_p > 0.0:
        dropout_mask = torch.zeros(M if x1 is None else 2 * M, N, device=x.device, dtype=torch.bool)
    else:
        dropout_mask = None
...
def _layer_norm_bwd(
    ...
    dx = (
        torch.zeros_like(x)
        if x_dtype is None
        else torch.zeros(M, N, dtype=x_dtype, device=x.device)
    )
    dresidual_in = (
        torch.zeros_like(x)
        if has_residual
    ...
    dx1 = torch.zeros_like(dx) if (has_x1 and dropout_p > 0.0) else None
    ...
    _dw = torch.zeros((sm_count, N), dtype=torch.float32, device=weight.device)
    _db = (
        torch.zeros((sm_count, N), dtype=torch.float32, device=bias.device)
        if bias is not None
        else None
    )
    _dw1 = torch.zeros_like(_dw) if weight1 is not None else None
    _db1 = torch.zeros_like(_db) if bias1 is not None else None
