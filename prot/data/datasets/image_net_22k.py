...
class ImageNet22k(ExtendedVisionDataset):
  ...
  def _load_entries_class_ids(self, root: Optional[str] = None) -> Tuple[List[_Entry], List[str]]:
    ...
    for class_index, class_id in enumerate(class_ids):
      ...
      if class_entries[-1].maybe_filename is None:
        raise RuntimeError(f'last block of class {class_id} has no filename')
    ...
    try:
      ...
      if len(data) >= 2 and tuple(data[:2]) == (0x1F, 0x8B):
        if index not in self._gzipped_indices:
           raise RuntimeError(f'unexpected gzip header for sample {index}.')
        with GzipFile(fileobj=BytesIO(data)) as g:
          data = g.read()
    except Exception as e:
      raise RuntimeError(
          f'can not retrieve image data for sample {index} '
          f'from {class_id} tarball from {e}')
  ...
  def _dump_entries(self, *args, **kwargs) -> None:
    ...
    for i, entry in enumerate(entries):
      ...
      entries_array[i] = (class_index, class_id, start_offset, end_offset, filename)
