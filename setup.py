...
BASE_WHEEL_URL = 'https://github.com/exogeny/prot/releases/download/{tag_name}/{wheel_name}'

# FORCE_BUILD: Force a fresh build locally, instead of attempting to find prebuilt wheels
# SKIP_CUDA_BUILD: Intended to allow CI to use a simple `python setup.py sdist` run to copy over raw files, without any cuda compilation
FORCE_BUILD = os.getenv('PROT_FORCE_BUILD', 'FALSE') == 'TRUE'
SKIP_CUDA_BUILD = os.getenv('PROT_SKIP_CUDA_BUILD', 'FALSE') == 'TRUE'
# For CI, we want the option to build with C++11 ABI since the nvcr images use C++11 ABI
FORCE_CXX11_ABI = os.getenv('PROT_FORCE_CXX11_ABI', 'FALSE') == 'TRUE'
...
def get_hip_version(rocm_dir):
  ...
  try:
    ...
    print(f'hip installation not found: {e} ROCM_PATH={rocm_path}')
    return None, None
...
def check_if_cuda_home_none(global_option: str) -> None:
  ...
  warnings.warn(
      ...
      'Are you sure your environment has nvcc available? If you\'re '
      'installing within a container from https://hub.docker.com/r/pytorch/pytorch,'
...
if not SKIP_CUDA_BUILD:
  ...
  ext_modules.extend([
      CUDAExtension(
          name="prot.ops._c.causal_conv1d_cuda",
          sources=[
      ...
      CUDAExtension(
          name="prot.ops._c.selective_scan_cuda",
          sources=[
...
def get_package_version():
  with open(os.path.join(os.path.dirname(__file__), 'prot', 'ops', 'causal_conv1d_interface.py'), "r") as f:
...
setup(
  ...
  url='https://github.com/exogeny/prot',
  classifiers=[
